...
pub struct Details {
    ...
    pub staked_tokens: Vec<StakedToken>,
}
...
impl Details {
    ...
    pub fn add_staked_token(&mut self, nft_mint: Pubkey, staker: Pubkey) -> Result<()> {
        self.staked_tokens.push(StakedToken { nft_mint, staker });
        Ok(())
    }

    pub fn remove_staked_token(&mut self, nft_mint: Pubkey) -> Result<()> {
        if let Some(index) = self
            .staked_tokens
            .iter()
            .position(|token| token.nft_mint == nft_mint)
        {
            self.staked_tokens.remove(index);
        }
        Ok(())
    }
